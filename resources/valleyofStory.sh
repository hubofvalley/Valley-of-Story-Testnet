...
- current story node version: ${CYAN}v1.7.0${RESET}
...
function update_geth() {
    ...
    bash <(curl -s https://raw.githubusercontent.com/hubofvalley/Valley-of-Story-Testnet/main/resources/story-geth_update.sh)
    menu
}
...
function deploy_validator_node() {
    ...
    bash <(curl -s https://raw.githubusercontent.com/hubofvalley/Valley-of-Story-Testnet/main/resources/story_validator_node_install_aeneid.sh)
    menu
}
...
function migrate_to_cosmovisor() {
    ...
    echo -e "${YELLOW}Press Enter to continue${RESET}"
    read -r
    bash <(curl -s https://raw.githubusercontent.com/hubofvalley/Valley-of-Story-Testnet/main/resources/cosmovisor_migration.sh)
    menu
}
...
function install_story_app() {
    ...
    mkdir -p story-v1.7.0
    wget -O story-v1.7.0/story https://github.com/piplabs/story/releases/download/v1.7.0/story-linux-amd64
    cp story-v1.7.0/story $HOME/go/bin/story
...
function apply_snapshot() {
    #echo -e "${YELLOW}This feature is not available yet.${RESET}"
    bash <(curl -s https://raw.githubusercontent.com/hubofvalley/Valley-of-Story-Testnet/main/resources/apply_snapshot.sh)
    menu
}
